...
class BaseController(BaseSearcher):
    ...
    def sample(self, decode=False):
        """
        Get a sampled architecture and its corresponding probabilities given current 
        controller policy parameters

        The generated architecture is the out-going information from controller to modeler. which in turn will feedback the reward signal for storage and training by the controller.

        Returns
        ----------
        onehots : list
            The sampled architecture sequence.  Each element of the one-hot vector corresponds to a
            specific component or parameter of the architecture. 
            In particular, the architecture sequence is ordered as::
        ...
        probs : list of ndarray
            The probabilities associated with each sampled operation and residual connection from the
            sampled architecture. 
            Shapes will vary depending on each layer's specification in ModelSpace for operation, and
            the layer number for residual connections.
            These probabilties indicate the likelihood or confidence of selecting each componenet 
            or parameter when generating the architecture. 
            Higher probabilities suggest a higher likelihood of choosing a particular option.
    ...
    def train(self):
        """
        Train the controller policy parameters for one step based on sampled architectures and
        their associated rewards that have been stored in the buffer
    ...
    def store(self, state=None, prob=None, action=None, reward=None):
        """
        Store all necessary information (e.g. sampled architecture and probability) and rewards 
        for a given architecture into a buffer

        This is the receiving method for controller to interact with modeler by storing the rewards 
        for a given architecture.
        
        The architecture and its probabilities can be generated by ``sample()`` method.
