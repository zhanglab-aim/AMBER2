...
from ..model_space import OperationSpace, Operation, ModelSpace, ModelVariable
...
class ProbaModelBuildGeneticAlgo(object):
    def __init__(self, model_space, buffer_size=1, batch_size=100, *args, **kwargs):
        ...
        if isinstance(model_space, ModelSpace):
            ...
            self.space_type = 'Model'
            for i in range(len(self.model_space)):
                assert isinstance(self.model_space[i], BayesProb)
        elif isinstance(model_space, OperationSpace):
            self.model_space = model_space
            self.space_type = 'Operation'
            assert isinstance(self.model_space, OperationSpace)
            for i in range(len(self.model_space)):
                assert len(self.model_space[i]) == 1, (
                    f"pmbga does not support more than 1 operations per layer; layer {i} has {len(self.model_space[i])}"
                ) 
            self.model_space_probs = self._get_probs_in_model_space()
        else:
            raise TypeError("model_space must be a valid search space")
    ...
    def sample(self):
        ...
        for i in range(len(self.model_space)):
            if self.space_type == 'Operation':
                ...
                arcs_tokens.append(self.model_space[i].sample())
    ...
    def _parse_probs_in_arc(self, arc, update_dict):
        for i in range(len(self.model_space)):
            ...
            if self.space_type == 'Operation':
                ...
                update_dict[self.model_space[i]].append(obs)
        return update_dict
